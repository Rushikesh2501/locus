import VitaminDtotal from "../../../../assets/images/vitaminDtotal.png";
import LipidProfile from "../../../../assets/images/lipidProfile.png";
import KidPro from "../../../../assets/images/kidPro.png";
import Lft from "../../../../assets/images/lft.png";
export default [
  {
    title: "VITAMIN",
    image: VitaminDtotal,
    route: "/sections/page-sections/features",
    description: "COMPLETE VITAMINS PROFILE",
    items: [
      {
        name: "VITAMIN D TOTAL",
        count: 10,
      },
      {
        name: "VITAMIN D2",
        count: 14,
      },
      {
        name: "VITAMIN D3",
        count: 8,
      },
      {
        name: "VITAMIN A",
        count: 1,
      },
      {
        name: "VITAMIN B-12",
        count: 11,
      },
      {
        name: "VITAMIN B1/THIAMIN",
        count: 11,
      },
      {
        name: "VITAMIN B2/TIBOFLAVIN",
        count: 6,
      },
      {
        name: "VITAMIN B3/NICOTINIC ACID",
        count: 3,
      },
      {
        name: "VITAMIN B5/PANTOTHENIC",
        count: 8,
      },
      {
        name: "VITAMIN B6/P5P",
        count: 6,
      },
      {
        name: "Logo Areas",
        count: 4,
      },
      {
        name: "VITAMIN B7/BITIN",
        count: 10,
      },
      {
        name: "VITAMIN B9/FOLIC ACID",
        count: 9,
      },
      {
        name: "VITAMIN E",
        count: 8,
      },
      {
        name: "VITAMIN K",
        count: 8,
      },
    ],
  },
  {
    title: "LIPID PROFILE",
    image: LipidProfile,
    route: "/sections/page-sections/features",
    description: "10 Lipid profile contents",
    items: [
      {
        name: "TOTAL CHOLESTEROL",
        count: 4,
      },
      {
        name: "HDL CHOLESTEROL",
        count: 2,
      },
      {
        name: "HDL/LDL RATIO",
        count: 3,
      },
      {
        name: "NON-HDL CHOLESTEROL",
        count: 3,
      },
      {
        name: "TC/HDL CHOLESTEROL RATIO",
        count: 3,
      },
      {
        name: "TRIG/HDL RATIO",
        count: 3,
      },
      {
        name: "TRIGLYCERIDES",
        count: 3,
      },
      {
        name: "VLDL CHELESTEROL",
        count: 3,
      },
    ],
  },
  {
    title: "KIDPRO",
    image: KidPro,
    route: "/sections/page-sections/features",
    description: "7 Renal profile contents",
    items: [
      {
        name: "BUN/SR.CREATININE RATIOs",
        count: 6,
      },
      {
        name: "BLOOD UREA NITROGEN (BUN)",
        count: 8,
      },
      {
        name: "CALCIUM",
        count: 3,
      },
      {
        name: "CREATININE-SERUM",
        count: 6,
      },
      {
        name: "UREA/SR.CREATININE RATIO",
        count: 6,
      },
      {
        name: "UREA (CALCULATED)",
        count: 6,
      },
      {
        name: "URIC ACID",
        count: 6,
      },
    ],
  },
  {
    title: "LIVER FUNCTION TEST",
    image: Lft,
    route: "/sections/page-sections/features",
    description: "12 Liver  profile contents",
    items: [
      {
        name: "SERUM ALB/GLOBULIN RATIO",
        count: 6,
      },
      {
        name: "ALKALINE PHOSPHATASE",
        count: 2,
      },
      {
        name: "BILIRUBUN (INDIRECT)",
        count: 2,
      },
      {
        name: "BILIRUBUN-TOTAL",
        count: 2,
      },
      {
        name: "GAMMA GLUTAMYL TRANSFERASE (GGT)",
        count: 2,
      },
      {
        name: "SGOT/SGPT RATIO",
        count: 1,
      },
      {
        name: "PROTEIN-TOTAL",
        count: 3,
      },
      {
        name: "ALBUMIN-SERUM",
        count: 4,
      },
      {
        name: "SERUM GLOBULIN",
        count: 3,
      },
      {
        name: "ASPARATE AMINOTRANSFERASE (SGOT)",
        count: 2,
      },
      {
        name: "ALANINE TRANSAMINASE (SGPT)",
        count: 2,
      },
    ],
  },
];
